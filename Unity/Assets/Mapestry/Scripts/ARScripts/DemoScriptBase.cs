...
{
    public abstract class AnchorBase : InputInteractionBase
    {
        ...
        [SerializeField]
        protected TMP_Text flowText  = null;
        ...
        public override void Start()
        {
            ...
            {
                Debug.Break();
                flowText.text = $"{nameof(CloudManager)} reference has not been set. Make sure it has been added to the scene and wired up to {this.name}.";
                return;
            }

            if (!SanityCheckAccessConfiguration())
            {
                flowText.text = $"{nameof(SpatialAnchorManager.SpatialAnchorsAccountId)}, {nameof(SpatialAnchorManager.SpatialAnchorsAccountKey)} and {nameof(SpatialAnchorManager.SpatialAnchorsAccountDomain)} must be set on {nameof(SpatialAnchorManager)}";
            }


            if (AnchoredObjectPrefab == null)
            {
                flowText.text = "CreationTarget must be set on the demo script.";
                return;
            }
        ...
        /// </returns>
        public abstract Task AdvanceFlowAsync();

        /// <summary>
        /// This version only exists for Unity to wire up a button click to.
        /// If calling from code, please use the Async version above.
        /// </summary>
        public async void AdvanceFlow()
        {
            try
            {
                isButtonClicked = true;
                advanceDemoTask = AdvanceFlowAsync();
                await advanceDemoTask;
            }
            catch (Exception ex)
            {
                Debug.LogError($"{nameof(AnchorBase)} - Error in {nameof(AdvanceFlow)}: {ex.Message} {ex.StackTrace}");
                flowText.text = $"Demo failed, check debugger output for more information";
            }
        }

        /// <summary>
        /// returns to the launcher scene.
        /// This method has been modified to turn off the camera for the map.
        /// </summary>
        ...
        protected void SetNearbyAnchor(CloudSpatialAnchor nearbyAnchor, 
                                    float DistanceInMeters, int MaxNearAnchorsToFind)
        {
        ...
        {
            Debug.Log("Locate pass complete - Cancelled?: "+args.Cancelled);
        }
        ...
        protected virtual void OnSaveCloudAnchorFailed(Exception exception)
        {
            ...
            UnityDispatcher.InvokeOnAppThread(() => this.flowText.text = string.Format("Error: {0}", exception.ToString()));
        }
        ...
        {
            if (IsPlacingObject() && isButtonClicked == false)
            {
        ...
        protected virtual async Task SaveCurrentObjectAnchorToCloudAsync(string anchorId)
        {
            ...
            while (!CloudManager.IsReadyForCreate)
            {
                ...
                flowText.text = $"Move your device to capture more environment data: {createProgress:0%}";
            }

            flowText.text = "Saving...";
        ...
        protected virtual GameObject SpawnNewAnchoredFindObject(Vector3 worldPos, Quaternion worldRot)
        {
            ...
            anchorToFind.transform.localScale = new Vector3(5, 5, 5);
        ...
        private void CloudManager_Error(object sender, SessionErrorEventArgs args)
        {
            ...
            UnityDispatcher.InvokeOnAppThread(() => this.flowText.text = string.Format("Error: {0}", args.ErrorMessage));
        }
